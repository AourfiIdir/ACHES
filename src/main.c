...
int achi_event2(int player,int board[3][3], int board_width,int board_height,int cellsize);//P2P PLACEMENT
int achi_event3(int player, int board[3][3], int board_width, int board_height, int cell_size);//P2P MOVEMENT
int achi_event4(int player, int board[3][3], int board_width, int board_height, int cell_size);//IA PLACEMENT
...
void renders_board(int board[3][3])
{
    ...
    // Draw board states (the game pieces)
...
void render_menu()
{
    ...
    const char *achi2 = "ACHI_IA";
    render_font(achi2,260,340);
    SDL_RenderPresent(render);
}
void render_page1()
{
...
int init_window(void)
{
    ...
    Mix_Music* btn = Mix_LoadMUS("button.mp3");
    if(btn==NULL){
        printf("error in loading the audio file");
    }
    Mix_PlayMusic(btn, -1); // -1 pour jouer en boucle
    */
...
int main(int argc, char *argv[])
{
    ...
    // Check for a winner after AIs move
    int winner_num = check_winner_board(board);
    if (winner_num != 0) {
        const char *winner;
        if (winner_num == PLAYER1) {
    ...
    //IA
    while (gaming == 0 && broke == 0 && current_page == GAME_ACHI2) { // Game loop for first option
    render_pages();

    if (check_winner_board(board) == 0) {
        if (turn == PLAYER1 && achi_event4(turn,board,300, 300, 100)) {
            // Change turn only after a valid move
            turn = PLAYER2;
            if (rounds > 0) {
                rounds--;
...
int achi_event4(int player, int board[3][3], int board_width, int board_height, int cell_size) {
    int x, y;
    SDL_Event event;
    ...
    if (SDL_PollEvent(&event)) {
        ...
        switch (event.type) {
        case SDL_QUIT:
            gaming = 1; // Signal to exit the game
            break;
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            if (x >= board_x && x <= board_x + board_width &&
                ...
                if (row >= 0 && row < 3 && col >= 0 && col < 3 && board[row][col] == 0) {
                    if (player == PLAYER1) {
                        board[row][col] = player; // Place PLAYER1's piece
                        printf("Player placed piece at (%d, %d)\n", row, col);
                        return 1; // Move was made, turn should change
                    }
                } else {
